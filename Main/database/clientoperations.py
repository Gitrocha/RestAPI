...
def client_test():
    ...
    connectors.update_age(emp_2, 37, conn)
    ...
    connectors.remove_employee(emp_4, conn)
    query4 = connectors.find_employee_exact('Rachel Higgs', conn)


    if len(query4) == 0:
        print('Query4, after removed employee 4, is the employee named Rachel Higgs'
              ' in not in DB anymore. Query Result:', query4)
    else:
        print('entities.Employee still in company DB')
